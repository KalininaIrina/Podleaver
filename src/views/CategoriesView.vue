<template>
  <div>
    <h1>Categories</h1>
    <button @click="getAll">🔍 Получить все</button>
    <button @click="getById">🔍 Получить по ID</button>
    <button @click="create">➕ Добавить</button>
    <button @click="update">✏️ Обновить</button>
    <button @click="remove">🗑 Удалить</button>
  </div>
</template>

<script>
export default {
  name: 'CategoriesView',
  methods: {
    async getAll() {
      const res = await fetch('http://localhost:8000/api/v1/categories');
      const data = await res.json();
      console.log('Все категории:', data);
    },
    async getById() {
      const id = 1;
      const res = await fetch(`http://localhost:8000/api/v1/categories/${id}`);
      const data = await res.json();
      console.log(`Категория #${id}:`, data);
    },
    async create() {
      const newData = { name: 'Новая категория' };
      const res = await fetch('http://localhost:8000/api/v1/categories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newData)
      });
      const data = await res.json();
      console.log('Создано:', data);
    },
    async update() {
      const id = 1;
      const updateData = { name: 'Обновленная категория' };
      const res = await fetch(`http://localhost:8000/api/v1/categories/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updateData)
      });
      const data = await res.json();
      console.log('Обновлено:', data);
    },
    async remove() {
      const id = 1;
      const res = await fetch(`http://localhost:8000/api/v1/categories/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) console.log(`Удалена категория #${id}`);
    }
  }
}
</script>
